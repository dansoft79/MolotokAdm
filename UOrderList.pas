...
begin
  ...
  begin
    ...
    begin
      ...
      cbWorkerCompany.EditValue := 'Ничего не указано';
...
//  ShowDatasetModal(qMaster);
  ...
  begin
    ...
    begin
      ...
      //по заданию от апреля 2023 добавляем подрядчика
      if AnsiLowerCase(FieldByName('OrgType').AsString) = AnsiLowerCase('самозанятый') then
      begin
        Name := Name + #13#10'Самозанятый';
      end
      else
      begin
...
{      if (FieldByName('ID_Worker').AsInteger <> 0) and (FieldByName('ID_Client').AsInteger <> 0) then
      ...
      //добавляем в шедулер, если не лид
